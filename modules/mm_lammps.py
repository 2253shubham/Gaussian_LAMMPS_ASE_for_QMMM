...
def create_atoms_in_lmp(atoms):
    ...
    for i, pos in enumerate(atoms.positions):
        atom_cmds.append('create_atoms {} single {} {} {}'.format(atoms.atom_type_list[i], pos[0], pos[1], pos[2]))
    for i in range(len(atoms)):
        atom_cmds.append('set atom {} charge {}'.format(i+1, atoms.arrays["charges"][i]))
    return atom_cmds
...
def get_atom_types(atoms):
    ...
    for i in range(len(uniq_atom_type)): # build dictionary to attach to ASE-atoms object
        atom_types_ase[uniq_atom_type[i]] = i+1
    atoms.atom_types = atom_types_ase
    atoms.atom_type_list = [atoms.atom_types[atom_types_pdb[i]] # assign atom types to each atom in ASE-atoms object
...
def get_bond_types(atoms):
    ...
    for i in range(len(atoms.ase_bonds[0])):
        for j in range(len(atoms.ase_bonds[0][i])):
            ...
            if (bonded_atom_types in unq_bonded_atom_types or bonded_atom_types[::-1] in unq_bonded_atom_types): # look for existing bond type
                bond_type = unq_bonded_atom_types.index(bonded_atom_types)+1 if bonded_atom_types in unq_bonded_atom_types else unq_bonded_atom_types.index(bonded_atom_types[::-1])+1
            else:
                bond_type = len(unq_bonded_atom_types)+1 # new bond type is always +1 existing bond types
    ...
    atoms.unq_bonds = np.array(unq_bonded_atom_types) # list of atom TYPES in unique bonds; len(unq_bonds) = # of types of bonds
...
def get_angle_types(atoms):
    ...
    for i in range(len(atoms.ase_angles[0])):
        for j in range(len(atoms.ase_angles[0][i])):
            ...
            if (angular_atom_types in unq_angular_atoms or angular_atom_types[::-1] in unq_angular_atoms):
                ang_type = unq_angular_atoms.index(angular_atom_types)+1 if angular_atom_types in unq_angular_atoms else unq_angular_atoms.index(angular_atom_types[::-1])+1
            else:
...
def get_dihedral_types(atoms):
    ...
    for i in range(len(atoms.ase_dihedrals[0])):
        for j in range(len(atoms.ase_dihedrals[0][i])):
            ...
            if (dihedral_atom_type in unq_dihedral_atoms or dihedral_atom_type[::-1] in unq_dihedral_atoms):
                dih_type = unq_dihedral_atoms.index(dihedral_atom_type)+1 if dihedral_atom_type in unq_dihedral_atoms else unq_dihedral_atoms.index(dihedral_atom_type[::-1])+1
            else:
...
def create_coeff_file(atoms, coeff_class_label):
    ...
    out2.truncate(0)
    infs_bonds = []
    import QM_MM_implemetation.bin_metd.parameters.all_atoms_LJ as all_atoms_LJ
    import all_bonds
    import all_angles
    import all_dihedrals
    for i, sym in enumerate(atoms.atom_types):
    ...
    for i in range(len(atoms.unq_bonds)):
        a1 = list(atoms.atom_types.keys())[list(atoms.atom_types.values()).index(atoms.unq_bonds[i][0])]
        a2 = list(atoms.atom_types.keys())[list(atoms.atom_types.values()).index(atoms.unq_bonds[i][1])]
        bond = a1+"_"+a2
    ...
    for i in range(len(atoms.unq_angles)):
        a1 = list(atoms.atom_types.keys())[list(atoms.atom_types.values()).index(atoms.unq_angles[i][0])]
        a2 = list(atoms.atom_types.keys())[list(atoms.atom_types.values()).index(atoms.unq_angles[i][1])]
        a3 = list(atoms.atom_types.keys())[list(atoms.atom_types.values()).index(atoms.unq_angles[i][2])]
        angle = a1+"_"+a2+"_"+a3
    ...
    for i in range(len(atoms.unq_dihedrals)):
        a1 = list(atoms.atom_types.keys())[list(atoms.atom_types.values()).index(atoms.unq_dihedrals[i][0])]
        a2 = list(atoms.atom_types.keys())[list(atoms.atom_types.values()).index(atoms.unq_dihedrals[i][1])]
        a3 = list(atoms.atom_types.keys())[list(atoms.atom_types.values()).index(atoms.unq_dihedrals[i][2])]
        a4 = list(atoms.atom_types.keys())[list(atoms.atom_types.values()).index(atoms.unq_dihedrals[i][3])]
        ...
        if any(pattern in dihedral for pattern in infs_bonds):
            ...
            print("delete_bonds all dihedral {} remove special".format(i+1), file=out2)
        else:
            ...
            print("dihedral_coeff {} {} {} {} {}".format(i+1,p1,p2,p3,p4), file=out)
    out.close()
    out2.close()
...
def get_charges(atoms):
    atoms.arrays["charges"] = np.zeros(len(atoms)) # initially all atoms are assigned zero charges
    import QM_MM_implemetation.bin_metd.parameters.all_atoms_LJ as all_atoms_LJ
...
def reset_positions(atoms_obj, ref):
    for j in range(3):
        atoms_obj.positions[:,j][atoms_obj.get_scaled_positions()[:,j]-ref[j]>0.5] = (atoms_obj.get_scaled_positions()[:,j][atoms_obj.get_scaled_positions()[:,j]-ref[j]>0.5]-1)*atoms_obj.cell.array.diagonal()[j]
        atoms_obj.positions[:,j][atoms_obj.get_scaled_positions()[:,j]-ref[j]<-0.5] = (atoms_obj.get_scaled_positions()[:,j][atoms_obj.get_scaled_positions()[:,j]-ref[j]<-0.5]+1)*atoms_obj.cell.array.diagonal()[j]
...
def reset_charges(atoms):
    ...
    for i in atoms.qm_idx:
        reset_charges_cmds.append('set atom {} charge {}'.format(i+1, atoms.arrays["charges"][i]))
    return reset_charges_cmds


def set_calc_params(atoms, class_label, coeff_class_label):
    header = []; lcmds = []; coeff_comds = []; run_comds = []
    ...
    return dict(lammps_header=header, lmpcmds=lcmds, create_box=False, create_atoms=False, boundary=False, run_cmds=run_comds)#, atom_types=atoms.atom_types, run_cmds=run_comds)#, coeff_cmds=coeff_comds, coeff_cmds, run_cmds # log_file=f'{sys_label}_aselammps.log'
...
def set_topology(atoms):
    ...
    for (t, i1, i2, i3, i4) in atoms.dihedrals:
        dihed_cmds.append('create_bonds single/dihedral {} {} {} {} {}'.format(t, i1, i2, i3, i4))
    return bond_cmds, angle_cmds, dihed_cmds


def unit_cell_coords(atoms_obj):
    for j in range(3):
        atoms_obj.positions[:,j][atoms_obj.positions[:,j]>atoms_obj.cell.array.diagonal()[j]] = atoms_obj.positions[:,j][atoms_obj.positions[:,j]>atoms_obj.cell.array.diagonal()[j]] - atoms_obj.cell.array.diagonal()[j]
        atoms_obj.positions[:,j][atoms_obj.positions[:,j]<0] = atoms_obj.positions[:,j][atoms_obj.positions[:,j]<0] + atoms_obj.cell.array.diagonal()[j]
...
#     # python tutorial for object based programming
    ...
    def hackaround_change_box(self, atoms):
        ...
        xh, yh, zh = box_dim
        xl, yl, zl = np.zeros(3)
        region = 'region mybox prism {} {} {} {} {} {} {} {} {}'.format(xl, xh, yl, yh, zl, zh, 0, 0, 0)
        create_box = 'create_box {} mybox bond/types {} angle/types {} dihedral/types {} extra/bond/per/atom {} extra/angle/per/atom {} extra/dihedral/per/atom {}'.format(n_types, nbond_types, nangle_types, ndihed_types, 50, 50, 50) # change extra/bond/angle/dihedral as per case to case basis
    ...
    def initialise_lammps(self, atoms):
        ...
        # Initialize cell
        self.set_cell(atoms, change = self.parameters.create_box) # different from original lammpslib function
    ...
    #        reset_charges_cmds = reset_charges(atoms)
        ...
        all_cmds = box_cmds+atom_cmds+mass_cmds+self.parameters.coeff_cmds+bond_cmds+angle_cmds+dihed_cmds+self.parameters.del_cmds+self.parameters.run_cmds#+reset_charges_cmds+self.parameters.run_cmds
        #print(all_cmds)
        for i in all_cmds:
            #print("[FROM ASE] : "+ i)
            self.lmp.command(i)
        ...
        self.lmp.command('thermo_style custom step etotal ke temp pe epair emol ebond eangle edihed eimp evdwl ecoul elong etail press vol pxx pyy pzz pxy pxz pyz')
    ...
    def propagate(self, atoms, properties, system_changes, n_steps, dt=None,
        ...
        if not self.started:
            ...
            if hasattr(atoms,"rt"): # check if the atoms object has some information on the optimization calculation type
    ...
    def redo_atom_types(self, atoms):
        current_types = set(
            ...
            in enumerate(atoms.arrays["atomtypes"].tolist()))
...
#       LAMMPSlib.start_lammps(self)
    ...
    def start_lammps(self):
        ...
        if self.parameters.log_file is None:
            ...
            cmd_args = ['-echo', 'screen', '-log', 'none', '-screen', self.parameters.log_file]

        self.cmd_args = cmd_args

        if self.lmp is None:
            self.lmp = lammps.lammps(self.parameters.lammps_name, self.cmd_args,
