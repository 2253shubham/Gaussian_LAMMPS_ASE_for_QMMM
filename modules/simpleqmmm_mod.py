...
import sys
from QM_MM_implemetation.bin_metd.modules.mm_lammps import reset_positions, unit_cell_coords
 ...
 #    them, so the second modification made is to read those data from the QM output file and incorporate them in LAMMPS (MM) calculations.
    ...
    def calculate(self, atoms, properties, system_changes):
        ...
        if len(self.qmatoms) > 0:
            ...
            print("QM Energy from Gaussian of QM subsystem = {}". format(self.qmcalc.results["energy"]), file=open_file1)
            if self.vacuum:
            ...
            print("MM Energy from LAMMPS of QM subsystem = {}". format(mm1_energy), file=open_file1)
            energy -= mm1_energy
        else:
            energy = 0
        ...
        print("MM Energy from LAMMPS of entire system = {}". format(mm2_energy), file=open_file1)
