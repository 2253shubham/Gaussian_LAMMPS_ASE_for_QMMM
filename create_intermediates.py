...
def parse_args():
    ...
    parser.add_argument('-d', '--dir', action='store', required=True) # directory to store neb.pdb files
    parser.add_argument('-n','--num_itd', action='store', default=8, type=int, required=True)
    parser.add_argument('-traj', '--traj_file', action='store', required=True, default='traj.traj') # trajectory input file
